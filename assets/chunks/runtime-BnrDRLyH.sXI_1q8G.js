...
</style>`,"</head><body>",'<div id="chart" class="blueprint-chart-container"></div>',o,"<script>",`var __BPC_SRC__ = ${l(e)};`,"function notifySize() {","  var h = document.documentElement.scrollHeight;",`  parent.postMessage({ type: "${i}", height: h }, "*");`,"}",...c,"notifySize();","new ResizeObserver(notifySize).observe(document.body);","<\/script>","</body></html>"].filter(Boolean).join(`
